...
<div class="fundraiser-page">
    ...
    <div class="search-area">
        <form method="GET" action="{{ route('admin.programs.index') }}" class="search-area">
    <input
        type="text"
        name="search"
        value="{{ $search ?? '' }}"
        ...
        <table class="fundraiser-table">
            ...
            <tbody id="programsTableBody">
                ...
                <tr id="row-{{ $program->id }}">
                    <td>{{ $index + 1 }}</td>
                    <td>{{ strtoupper($program->title) }}</td>
                    <td>{{ $program->event_date ? \Carbon\Carbon::parse($program->event_date)->format('d/m/Y') : 'XXX' }}</td>
                    <td>{{ $program->event_time ? \Carbon\Carbon::parse($program->event_time)->format('h:i A') : 'XXX' }}</td>
                    <td>{{ strtoupper($program->location ?? 'XXX') }}</td>
                    <td>{{ strtoupper($program->theme ?? 'XXX') }}</td>
                    <td>
                        <label class="toggle-switch">
                            <input type="checkbox" 
                                   onchange="toggleDisplay({{ $program->id }})" 
                                   {{ $program->is_visible ? 'checked' : '' }}>
                            <span class="toggle-slider"></span>
                        </label>
                    </td>
                    <td>
                        <a href="{{ route('admin.programs.show', $program->id) }}" class="icon-btn info-btn" title="View Full Details">
                            <img src="/assets/icons/info.png" alt="Info">
                        </a>
                    </td>
                    <td class="action-buttons">
                        <button class="icon-btn edit-btn" onclick="openEditModal({{ $program->id }})" title="Edit Basic Info">
                            <img src="/assets/icons/update.png" alt="Edit">
                        </button>
                        <button class="icon-btn delete-btn" onclick="confirmDelete({{ $program->id }}, '{{ addslashes($program->title) }}')" title="Delete">
